...
比较门店定位和变化轨迹，识别四象限迁移及风险集中区域。

![门店组合的变化轨迹与迁移摘要](docs/images/store-portfolio.jpg)
...
- **门店组合分析**：当前期、对比期和变化轨迹；广告及促销 × 客户贡献四象限；风险门店和差异排名。
- **单店分析**：既有门店和新店画像、核心指标、明细损益表、广告及促销构成与变动。
...
中英文界面也共用同一套 Core：翻译层只在渲染边界替换文案，不修改数据 schema、计算 key 或四象限 canonical value。
